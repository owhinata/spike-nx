...
- **main**: リリースブランチ。線形履歴（マージコミットなし）
- **develop**: 開発ブランチ。feature ブランチのマージ先
- **feature**: `feat/`, `docs/`, `style/`, `fix/`, `build/`, `refactor/`, `chore/` prefix。ベースは常に `develop`

### コミット

conventional commits 形式 `type: short description`
...
```bash
gh pr create --base develop --title "type: short description" --body "$(cat <<'EOF'
## Summary
- 変更点を箇条書き
...
rebase マージで線形履歴を維持。**develop ブランチは削除しない**。
