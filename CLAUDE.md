...
**Phase 系 / architecture-changing plan は plan 確定前 + 実装後の両方で codex-review を実施する** (memory `feedback_codex_review_implementation_loop` 由来、Phase 3 から運用)。

対象となる plan:
- `~/.claude/plans/*.md` 以下に作成する Phase plan (Phase 3a/3b、Phase 6、今後の Phase 7 等)
- 新規 driver / ペリフェラル採用、PID / 制御アルゴリズム構造変更を伴う設計文書
- chardev ABI、cfg key 拡張など複数 layer に影響する変更計画
- 多 layer 横断する Issue (例: HW + driver + アプリ) の実装方針

ゲートのタイミング:

1. **Plan 確定前** (実装着手前): plan ドラフトを `codex-review` skill で 3 面 (設計 / MCU 実機能 / HW リソース競合) review、BLOCKING / CONCERN を全て解消してから `ExitPlanMode` で plan 確定
2. **実装後** (commit 前): branch の diff を `codex-review` skill で再 review、BLOCKING を全て解消してから user に HW verify 依頼 → commit

`codex-review` skill は `.claude/skills/codex-review/` で定義済 (sandbox は `danger-full-access`、approval-policy は `never`、memory `reference_codex_sandbox` 参照)。Codex は複数 round 必要なことが多い (Phase 3 plan = 7 round、Phase 6 plan = 5 round LGTM)。

**Plan を書いたら codex-review を skip しない**。skip すべきと判断する場合も、ユーザに必ず確認してから skip する。
