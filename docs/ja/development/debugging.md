...
!!! warning "LEGO Powered Up port 有効時の制限 (Issue #80)"
    `CONFIG_LEGO_PORT=y` (defconfig で既定) は Port E (PC6/PC7) を TIM3 PWM 用に占有するため、USART6 を disable する必要がある。これにより以下が**完全に失われる**:

    - `up_lowputc()` 経由の low-level boot 出力 (NuttX 起動失敗時の panic message)
    - Pre-USB stack debug (NSH 起動前の早期 init)
    - HardFault / panic 後の post-mortem dump で USB が止まっている場合のフォールバック

    SPIKE Prime Hub は SWD も使えないため、emergency / early / panic debug の物理経路は **存在しない**。日常デバッグは USB CDC + RAMLOG + coredump で十分だが、起動失敗の解析が必要になった場合は `CONFIG_LEGO_PORT=n` でビルドし直して USART6 を一時的に有効化する必要がある。
...
| Last resort | USART6 (LEGO_PORT disable 時のみ) — I/O ポート UART を一時的にデバッグ出力に使用 | USB 未動作時 |
