...
| 項目 | 対応 | 理由 |
|---|---|---|
| `CONFIG_APP_LED` 無効化 | 別 issue で対応 | `led_main.c` が `tlc5955_set_duty()` を直呼び。`/dev/rgbled` 風 char driver 経由へ refactor 必要 |
...
| `pytest -m "not slow and not interactive"` | ✅ **28 passed**, 2 expected failures (LED 関連、`APP_LED` 無効化の派生) |
...
- **APP_LED の refactor** ([#39](https://github.com/owhinata/spike-nx/issues/39)): `tlc5955_set_duty()` 直呼びを `/dev/rgbled` 風 char driver 経由に置き換える。
