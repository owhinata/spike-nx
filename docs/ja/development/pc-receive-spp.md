...
| `SET ODR <hz>\n` | ODR 変更 (13/26/52/104/208/416/833 Hz) | live SET 可 (streaming 中も可)。**>833 は `ERR invalid_odr`** |
| `SET ACCEL_FSR <g>\n` | 加速度 FSR (2/4/8/16) | live SET 可 |
| `SET GYRO_FSR <dps>\n` | ジャイロ FSR (125/250/500/1000/2000) | live SET 可 |
| `GET ODR\n` | 現在の ODR を **enum idx** で取得 | reply `OK <idx>\n` |
| `GET ACCEL_FSR\n` | 現在の加速度 FSR を **enum idx** で取得 | reply `OK <idx>\n` |
| `GET GYRO_FSR\n` | 現在のジャイロ FSR を **enum idx** で取得 | reply `OK <idx>\n` |

Issue #139 で SET の "IMU OFF 時のみ" 制約は撤去。`struct sensor_imu` に `odr_idx` / `fsr_xl_idx` / `fsr_gy_idx` を埋め込んで sample 自身が自己記述するようになり、`bundle_emitter` が同一 frame 内 idx 不一致を検出して frame を分割発出することで、streaming 中の live SET も BUNDLE wire 一貫性を保ったまま反映される (SET 直後 1 tick で最大 2-3 frame 連続発出 → BTstack queue で吸収)。

GET の reply は driver-internal な enum idx (`lsm6dsl_odr_e` / `lsm6dsl_fsr_xl_e` / `lsm6dsl_fsr_gy_e`) をそのまま 10 進数で返す。host 側は対応する lookup table で物理値 (Hz / g / dps) に変換する (sample 埋込 idx と同一表現)。
...
Hub -> PC:  OK\n
            ...
            (BUNDLE header の rate/fsr は samples[0] の idx から source)
PC -> Hub:  SET GYRO_FSR 500\n      (streaming 中の live 変更も OK)
Hub -> PC:  OK\n
            ... 直後の 1 tick で frame 分割発出 (idx 切替境界), 以降は新 FSR で継続 ...
PC -> Hub:  IMU OFF\n
PC -> Hub:  SENSOR OFF\n
Hub -> PC:  OK\n
Hub -> PC:  OK\n
