...
- `0` = **UNKNOWN/UNSET** — 予約済みのセンチネルで、有効な LEFT/RIGHT
  ではない。edge の概念より前のファームウェアのトレースにのみ現れる。
  Issue #180 以降、デーモンは `0` を書かない。
- `1` = **LEFT** (既定 / 従来の操舵極性)
- `2` = **RIGHT**

Issue #180 以降、PID デーモンは選択エッジ (`1`/`2`、既定は LEFT。
`linetrace edge` 参照) を常に記録する。P0c のオフライン fitter は依然
として **`edge == 0` を実エッジとして解釈してはならない**。そうしたレガシー
キャプチャでは `c` の符号 (どちらのエッジか) はオペレータが帯域外で指定する。
