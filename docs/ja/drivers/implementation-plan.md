...
- STM32F413 固有の workaround: `STM32_HAVE_DAC1` が select されていないため RCC を直接 enable、`stm32f413xx_pinmap.h` に DAC1 OUT1 マクロがないため board ローカルで `GPIO_DAC1_OUT1_F413` を定義
