...
### 3.5.4 飽和 / LOST ステートマシン (Issue #169 HW-verify fix)

線形帯域を外れると innovation が嘘になるため、観測器にゴミを積分させない。P1a の初回実機走行は **両エッジ極性でラインから発散**した。真因は、測定モデルが ±`sat_band` 帯域内でしか成立しないことに加えて、飽和検出が **ハード** 0/1024 rail のみで判定していた(白寄りの 975 を誤って correct していた)こと、および LOST トリガが **連続** SAT tick を数えていた(SAT↔NORMAL flap で streak がリセットされ LOST が一度も発火しなかった)こと。帯域外の 1 tick が 1 回の correct で数 mm の `e_y` を注入し暴走した。修正:

- **NORMAL** — 測定が band-margin 窓 `[black+margin, white-margin]` 内かつ `|ey_p| ≤ sat_band`: Kalman correct を実行するが、先に innovation を `±|c_signed|·sat_band` に **クランプ**。これで帯域境界の測定でも 1 tick で `e_y` に注入できるのは ~`sat_band` までに制限される。
- **SATURATED** — その窓の外(または `|ey_p| > sat_band`): 観測器を **freeze**(predict のみ、correct を skip)し直近の良い姿勢で steer 継続、帯域外 **時間** を累積。累積飽和時間が `lost_time_ms` を超えると LOST へ。連続 tick ではなく時間ベースなので SAT↔NORMAL flap で LOST を defeat できない。
...
飽和検出は動作 target では look-ahead 輝度がハード rail より手前で線形帯域を外れるため、ハード 0/1024 rail ではなく **実測センサレンジ**(`--black` / `--white` / `--band-margin`)で判定する。oracle で検証済み: `python3 tools/lqr_linetrace/lqr_sim.py --report satband --c 150 --L 55 --target 634` は修正前 SM が回復可能な帯域外キックで発散(`peak|innov|≈960`)する一方、修正後 SM は ~0.1 mm まで回復(`innov` ≈14 にクランプ)し、真の loss では LOST→seek→auto-stop が発火することを示す。

**動作 target。** P0a は edge midpoint を ≈634(black ≈249 / white ≈1020)と実測した。PID setpoint は 512 のままだが、512 では black rail がライン中心から ~1.7 mm しか離れておらず帯域は ±3.4 mm なので、通常の look-ahead 由来のスポット移動でも飽和し **SM に依らず**観測器がラインを失う。oracle は修正後 SM でも target=512 では帯域を追従できず、対称な midpoint では綺麗に追従することを確認している。よって LQG は target をデフォルトで midpoint(`LQG_DEF_TARGET=634`)にする。PID は不変。
...
`target` のデフォルトは実測 edge midpoint(634)で PID の 512 ではない — 観測器は対称な midpoint を必要とする。
...
| `target` | 634 | LQG 動作 setpoint = 実測 edge midpoint(PID は 512 のまま) |
| `edge` | right | steer 極性 |
| `q1r` | 0.888 | q1/r(state-feedback の硬さ) |
| `q2` | 0 | 姿勢角重み(0 → 全速度 zeta 0.707) |
| `kf_qpos` / `kf_qhead` / `kf_rmeas` | 1.0 / 1e-4 / 49 | Kalman Q[0][0] / Q[1][1] / R |
| `sat_band` | 3.4 | 線形帯域の半幅 mm(innovation クランプの尺度 `|c|·sat_band` も兼ねる) |
| `black` / `white` / `band_margin` | 249 / 1020 / 100 | 実測センサレンジ。`[black+margin, white-margin]` の外で飽和 |
| `lost_time_ms` | 50 | 累積帯域外時間 → LOST |
| `seek_dps` | = base speed | LOST seek の大きさ |
| `lost_timeout_ms` | 500 | LOST → auto-stop |
| `lost_ticks` | 5 | CLI 互換のため保持(LOST は `lost_time_ms` で判定) |
