...
`drivebase_daemon.c` の publish overwrite が `latched == 1D` (走行中) または `requested == 1D && origin_valid` (未走行で `set-gyro` 直後) なら `st.angle_mdeg = (int32_t) CLAMP(raw - gyro_origin_mdeg, INT32_MIN, INT32_MAX)` で IMU 由来値に差し替え。PID 入力と user-visible publish が同 origin を引くため SSOT 化される。

### Out of scope (別 Issue)

- X/Y bias EMA 温度ドリフト追従 — Phase 3a Codex CONCERN 2 領分
- 3D mode (`set-gyro 3d`) — Phase 7 までは `-ENOSYS`
