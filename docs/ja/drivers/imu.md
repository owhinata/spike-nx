...
B-L4S5I-IOT01A Discovery ボード上の LSM6DSL を開発ターゲットとして採用。最終目標は pybricks の LSM6DS3TR-C ドライバと同等の機能を NuttX 上で実現すること。
...
LSM6DSL/LSM6DS3TR-C は 3 軸加速度計と 3 軸ジャイロスコープを統合したシングルダイ 6 軸 IMU。
...
### SPIKE Prime Hub (最終ターゲット)

| ピン | 機能 | 説明 |
|---|---|---|
| PB10 | I2C2_SCL | I2C クロック |
| PB3 | I2C2_SDA | I2C データ (AF9) |
...
処理ライブラリ (`apps/imu/`) のセンサーフュージョン・キャリブレーションコードは変更不要。uORB トピックインターフェースが同一のため、ドライバのみ差し替え。

## 12. IMU 処理ライブラリ (apps/imu/)
...
| `imu.acceleration()` | uORB `sensor_accel0` 直接読み出し | mm/s^2 単位 |
