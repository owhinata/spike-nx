...
APB1 タイマーは APB1 prescaler > 1 の場合クロックが 2 倍になる (48 MHz x 2 = 96 MHz)。全チャンネル反転 PWM 設定。pybricks は 12 kHz を使用 (LEGO 公式は 1.2 kHz)。
...
PWM チャンネルを常時有効のままにでき、NuttX 標準 PWM ドライバとの親和性が高い。
...
NuttX は PWM ドライバ (`/dev/pwmN`) を提供するが、モーター制御フレームワークは存在しない。
...
- `pybricks/lib/pbio/src/servo.c` -- PID 制御ループ
- `pybricks/lib/pbio/src/observer.c` -- ルーエンバーガーオブザーバー
