...
CC2564C の bring-up (slow clock → nSHUTD → HCI_Reset @ 115200 → 0xFF36 baud 切替 → init script @ 3 Mbps → `btuart_register`) が成功し、NuttX の BT スタックに `bnep0` netdev として登録されているかを検証する。詳細は [Bluetooth ドライバ](../drivers/bluetooth.md) を参照。

### H-1: test_bt_bringup_dmesg

- **目的**: bring-up 完了バナー確認 + 失敗ログ無し
- **コマンド**: `dmesg`
- **判定**: `BT: CC2564C ready at 3000000 bps` を含み、`btuart_register failed` および `stm32_bluetooth_initialize() failed` を含まない

### H-2: test_bt_netdev_visible

- **目的**: BT netdev が NuttX ネットインタフェースとして登録されていること
- **コマンド**: `ifconfig`
- **判定**: 出力に `bnep0` を含む

### H-3: test_bt_info_bdaddr

- **目的**: BT コントローラが正当な BD アドレスを返すこと (HCI_Read_BD_ADDR)
- **コマンド**: `bt bnep0 info`
- **判定**: 48-bit BD アドレス (`xx:xx:xx:xx:xx:xx`) が取得でき、`00:00:...` / `ff:ff:...` でない

### H-4: test_bt_info_buffer_pool

- **目的**: HCI bring-up が完了し、ACL バッファプールが正の値であること (bring-up 失敗時は silently 0 になるため、H-3 だけでは検出できない)
- **コマンド**: `bt bnep0 info`
- **判定**: `Free: <N>` が `N > 0`、`Max ACL: <M>` が `M > 0`

### H-5: test_bt_scan `@interactive`

- **目的**: HCI inquiry で周囲の BT デバイスを発見できること
- **前提**: スマートフォンなど BT デバイスが近傍で advertising 状態になっていること
- **コマンド**: `bt bnep0 scan start` → 3 秒待機 → `bt bnep0 scan get` → `bt bnep0 scan stop`
- **判定**: `bt bnep0 scan get` の出力に 1 件以上の BT アドレスが含まれる + operator が目視確認
- **依存**: 近傍の BT デバイス (スマホ等)
